...
export class OracleUpdater {
  ...
  private async checkAndUpdate(): Promise<void> {
    try {
      ...
      console.log(
        `[ORACLE] Found ${pricesByWindow.size} window(s) with ${result.rows.length} total price(s)`,
      );

      // Update contract for each window (serially to avoid sequence number collisions)
      for (const [key, data] of pricesByWindow) {
        const success = await this.updateContract(
          data.symbol,
          data.prices,
          data.window_start,
        );
  ...
  private async updateContract(
    ...
    try {
      ...
      const pricesAsI128 = prices.map((price) => {
        ...
        return nativeToScVal(Math.round(priceAsNumber * 100000000), {
          type: "i128",
        });
      });
      ...
      const transaction = new TransactionBuilder(sourceAccount, {
        ...
        .addOperation(
          contract.call(
            "update_prices",
            nativeToScVal(symbol.toUpperCase().slice(0, 10), {
              type: "symbol",
            }),
      ...
      if (sendResponse.status === "PENDING") {
        ...
        if (getResponse.status === "SUCCESS") {
          console.log(
            `[ORACLE] ✅ TX ${txHash}: ${symbol} window=${windowTimestamp} with ${prices.length} price(s): [${prices.join(", ")}]`,
          );
          ...
          try {
            ...
            console.error(
              `[ORACLE] Failed to save TX hash to database:`,
              dbError,
            );
          }
        ...
        console.error(
          `[ORACLE] ❌ TX ${txHash} rejected immediately:`,
          sendResponse,
        );
        return false;
      }
